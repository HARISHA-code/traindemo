...
func main() {
	...
	// Retrieve and display updated seat allocation details
	updatedSeatAllocations, err := getSeatAllocation(c, "B")
	if err != nil {
		log.Fatalf("Error getting updated seat allocation details: %v", err)
	}
	fmt.Println("Updated Seat Allocation Details:")
	for _, allocation := range updatedSeatAllocations {
		fmt.Printf("User: %s, Section: %s, Seat: %s\n", allocation.UserEmail, allocation.Section, allocation.SeatNumber)
	}
